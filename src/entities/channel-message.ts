...
class ChannelMessage extends Message {
  ...
  static toDTO(
    message: ChannelMessageDocument,
    serverId: string,
    groupId: string
  ) {
    ...
    dtoMessage.reactions = message.reactions.map((reaction: ReactionDocument) =>
      Reaction.toDTO(reaction)
    );
    return dtoMessage;
  }
}

export type ChannelMessageDocument = Document<any, any, ChannelMessage> &
  ChannelMessage;
