...
export class ChannelsService {
  ...
  async createChannel(
    ...
    serverId: string,
    groupId: string,
    newChannelRequest: Pick<Channel, "name" | "type">
  ) {
    ...
    const newChannelDto = Channel.toDTO(
      group.channels[length - 1] as ChannelDocument,
      serverId,
      groupId
    );
  ...
  async updateChannel(
    ...
    groupId: string,
    channelId: string,
    channelUpdate: UpdateChannelRequest
  ) {
    const channel = await this.getByIdAndType(
      userId,
      serverId,
      groupId,
      channelId
    ...
    const group2 = await this.groupsService.getById(
      userId,
      serverId,
      channelUpdate.groupId
    );
    ...
    if (channelUpdate.order !== undefined) {
      ...
      if (groupId === channelUpdate.groupId) {
        channels = group1.channels = changeDocumentOrder(
          group1.channels as ChannelDocument[],
          channelId,
          channelUpdate.order
        );
      } else {
        const sortedChannels1 = sortDocuments(
          group1.channels as ChannelDocument[]
        );
        const sortedChannels2 = sortDocuments(
          group2.channels as ChannelDocument[]
        );
        const index1 = sortedChannels1.findIndex(
          (channel) => channel._id.toString() === channelId
        );
        sortedChannels2.splice(
          channelUpdate.order,
          0,
          sortedChannels1.splice(index1, 1)[0]
        );
  ...
  async deleteChannel(
    ...
    const index = group.channels.findIndex(
      (channel) => channel._id.toString() === channelId
    );
    ...
    this.socketIoService.channelDeleted(
