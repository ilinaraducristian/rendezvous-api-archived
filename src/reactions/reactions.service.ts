...
export class ReactionsService {
  ...
  async createReaction(
    ...
    const { message, channel } = await this.channelMessagesService.getById(
      userId,
      serverId,
      groupId,
      channelId,
    ...
    if (newReaction.serverEmoji) {
      ...
      const emoji = server.emojis.find(
        (emoji) => emoji._id.toString() === newReaction.emoji
      );
  ...
  async deleteReaction(
    ...
    const { message } = await this.channelMessagesService.getById(
      userId,
      serverId,
      groupId,
      channelId,
